...

{
    ...
    if($code !="")
        $data = $wpdb->get_results( $wpdb->prepare("SELECT * FROM " . $wpdb->prefix. TNADVS_TABLE_NAME.  " WHERE vcode = %s and vlan= %s  and vstatus ='publish'  order by $orderby ",array($code,$lan)));
    else
    {
        $data = $wpdb->get_results( $wpdb->prepare("SELECT * FROM " . $wpdb->prefix. TNADVS_TABLE_NAME.  " WHERE  vlan=%s and vstatus ='publish'   order by $orderby" ,$lan));
    }
    return $data;
}
